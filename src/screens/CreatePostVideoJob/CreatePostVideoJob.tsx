...
import { StyleSheet, View, ScrollView, Image, Text as TextRN, SectionList } from 'react-native';
import { Ribbon, Title, Text, TextLink, Checkbox, Button,Input, Avatar } from '../../components';
...
type CreateVideoJobsProps = StackScreenProps<any, any> & {
}
...
const CreatePostVideoJob: React.FC<CreateVideoJobsProps> = () => {
  const [se, setSe] = useState();
  const [youPost, setYouPost] = useState(true);
  const [companyPost, setCompanyPost] = useState(false);
  ...
  return (
    <>
      ...
      <ScrollView style={[styles.container]}>
        <Title bold position='center' size='small'>Publica tu oferta laboral</Title>
      ...
      <Image style={{height:'100%',width:"40%"}}
       source={{uri:imageAvatar}}></Image>
        ...
        <Checkbox
              ...
              value={youPost}
              type="secondary"
              onChangeValue={(value) => { setYouPost(value); setCompanyPost(false) }} />
               <Checkbox
              label="Empresa o organizacion"
              value={companyPost}
              type="secondary"
              onChangeValue={(value) => { setCompanyPost(value); setYouPost(false)}} />
                {companyPost?  <Input  label='Contratante' placeholder='Nombre de la empresa'></Input> : null}
                {/*select tipo de empleo*/}
       
        <Input  label='Ubicación' placeholder='Lugar de labor'></Input>
...
*/}
        ...
        <Button marginBottom={50}>Publicar</Button>
      </ScrollView>
    </>
  );
}else{return(
  ...
  <Ribbon backgroundColor='white' backDefaultColor={colors.gray} title="Publicar" />
  <ScrollView style={[styles.container]}>
    <Title bold position='center' size='small'>Datos del videoempleo</Title>
   <Box style={[stylesCardCv.inLine]}>
    {/* select*/ }<Input  label='Salario Max' placeholder='Salario Minimo'></Input>
    
    <Input  label='Salario Min' placeholder='Salario Maximo'></Input>
    </Box>
    <Box>
    <Title bold position='center' size='small'>Tipo de salario</Title>
...
export default CreatePostVideoJob;
